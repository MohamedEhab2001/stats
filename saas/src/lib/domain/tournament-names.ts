// Cosmetic tournament name pool, used to flavor auto-created monthly/yearly tournaments.
...
export function randomTournamentName(used: string[] = []): string {
  const recent = new Set(used.slice(-50))
  const fresh = TOURNAMENT_NAMES.filter((n) => !recent.has(n))
  const pool = fresh.length ? fresh : TOURNAMENT_NAMES
